      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden md:flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs">
          ...
          <select
            ...
            className="bg-transparent text-slate-800 font-semibold focus:outline-none cursor-pointer"
          >
            {STATES_DISTRICTS[activeFilterState]?.map((d) => (
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-xl py-2 z-50">
              <div className="px-4 py-2 border-b border-slate-100">
                ...
                <div className="mt-1 flex items-center gap-1 text-[10px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded w-max">
                  <ShieldCheck className="w-3 h-3" />
                  <span>Authenticated Official</span>
                </div>
              </div>
              <button
                onClick={logout}
